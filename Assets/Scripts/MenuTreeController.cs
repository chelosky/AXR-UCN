...
public class MenuTreeController : MonoBehaviour
{
    ...
    private int buttonDistance = 150;//distancia entre botones
    ...
    public void OpenMenu(){
        ...
        float baseAngle = 270 * Mathf.Deg2Rad; //ANGULO BASE (180°)
        this.openMenu = !this.openMenu;
        for(int i=0;i <= this.childButtons.Count-1;i++){
            if(this.openMenu){
                float xpos = Mathf.Cos(baseAngle) * this.buttonDistance*(i+1);
                float ypos = Mathf.Sin(baseAngle) * this.buttonDistance*(i+1);
